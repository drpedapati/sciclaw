...
func createToolRegistry(workspace string, restrict bool, cfg *config.Config, msgBus *bus.MessageBus, profile ToolProfile) *tools.ToolRegistry {
	...
	docxReviewDiffTool := tools.NewDOCXReviewDiffTool(workspace, restrict)
	docxReviewDiffTool.SetSharedWorkspacePolicy(sharedWorkspace, sharedReadOnly)
	docxReviewApplyTool := tools.NewDOCXReviewApplyTool(workspace, restrict)
	docxReviewApplyTool.SetSharedWorkspacePolicy(sharedWorkspace, sharedReadOnly)
	xlsxReviewReadTool := tools.NewXLSXReviewReadTool(workspace, restrict)
	xlsxReviewReadTool.SetSharedWorkspacePolicy(sharedWorkspace, sharedReadOnly)
	xlsxReviewDiffTool := tools.NewXLSXReviewDiffTool(workspace, restrict)
	xlsxReviewDiffTool.SetSharedWorkspacePolicy(sharedWorkspace, sharedReadOnly)
	xlsxReviewApplyTool := tools.NewXLSXReviewApplyTool(workspace, restrict)
	xlsxReviewApplyTool.SetSharedWorkspacePolicy(sharedWorkspace, sharedReadOnly)
	pptxReviewReadTool := tools.NewPPTXReviewReadTool(workspace, restrict)
	pptxReviewReadTool.SetSharedWorkspacePolicy(sharedWorkspace, sharedReadOnly)
	pptxReviewDiffTool := tools.NewPPTXReviewDiffTool(workspace, restrict)
	pptxReviewDiffTool.SetSharedWorkspacePolicy(sharedWorkspace, sharedReadOnly)
	pptxReviewApplyTool := tools.NewPPTXReviewApplyTool(workspace, restrict)
	pptxReviewApplyTool.SetSharedWorkspacePolicy(sharedWorkspace, sharedReadOnly)
	...
	registry.Register(docxReviewApplyTool)
	registry.Register(xlsxReviewReadTool)
	registry.Register(xlsxReviewDiffTool)
	registry.Register(xlsxReviewApplyTool)
	registry.Register(pptxReviewReadTool)
	registry.Register(pptxReviewDiffTool)
	registry.Register(pptxReviewApplyTool)
...
func NewAgentLoopWithOptions(cfg *config.Config, msgBus *bus.MessageBus, provider providers.LLMProvider, opts LoopOptions) *AgentLoop {
	...
	if profile == ToolProfileDefault {
		...
		if cfg.EffectiveMode() == config.ModePhi {
			subagentManager.SetToolResultMaxChars(localToolResultMaxChars)
		}
