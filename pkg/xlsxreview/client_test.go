...
func TestClientReadParsesJSON(t *testing.T) {
	...
	client := newClientWithRunner(ClientOptions{
		...
		_ = ctx
		if binary != "/usr/local/bin/xlsx-review" {
			t.Fatalf("unexpected binary: %s", binary)
		}
		gotArgs = append([]string{}, args...)
		return RunResult{Stdout: `{"workbook":{"sheet_count":1},"sheets":[{"name":"Data"}],"warnings":["note"]}`}, nil
	})
	...
	if len(result.Sheets) == 0 || len(result.Workbook) == 0 || len(result.Warnings) != 1 {
...
func TestClientDiffParsesJSON(t *testing.T) {
	workspace := t.TempDir()
	oldPath := filepath.Join(workspace, "old.xlsx")
	newPath := filepath.Join(workspace, "new.xlsx")
	for _, p := range []string{oldPath, newPath} {
		if err := os.WriteFile(p, []byte("xlsx"), 0o644); err != nil {
			t.Fatalf("write xlsx: %v", err)
		}
	}
	var gotArgs []string
	client := newClientWithRunner(ClientOptions{
		...
		_ = binary
		gotArgs = append([]string{}, args...)
		return RunResult{Stdout: fmt.Sprintf(`{"old_file":%q,"new_file":%q,"cell_changes":[{"sheet":"Data","cell":"A1"}],"formula_changes":[],"metadata_diff":{},"sheets_diff":{},"structure_diff":{},"summary":{"identical":false}}`, oldPath, newPath)}, nil
	})
	...
	if result.OldFile != oldPath || result.NewFile != newPath || len(result.CellChanges) == 0 {
...
func TestClientApplyDryRunAllowsEmptyOutputPath(t *testing.T) {
	...
	client := newClientWithRunner(ClientOptions{
		...
		_ = binary
		gotArgs = append([]string{}, args...)
		return RunResult{Stdout: fmt.Sprintf(`{"input":%q,"author":"Reviewer","changes_attempted":1,"changes_succeeded":1,"comments_attempted":0,"comments_succeeded":0,"results":[{"index":0,"type":"set_cell","success":true,"message":"Validated"}],"success":true}`, inputPath)}, nil
	})
...
func TestClientApplyRejectsOutputEqualToInput(t *testing.T) {
	...
	client := NewClientWithOptions(ClientOptions{LookPathFn: func(file string) (string, error) { return "/usr/local/bin/xlsx-review", nil }})
...
func TestClientResolveBinaryPathPrefersExplicitEnvOverride(t *testing.T) {
	workspace := t.TempDir()
	override := filepath.Join(workspace, "xlsx-review")
	if err := os.WriteFile(override, []byte("#!/bin/sh\n"), 0o755); err != nil {
		t.Fatalf("create override: %v", err)
	}
	t.Setenv("PICOCLAW_XLSX_REVIEW_BINARY", override)
	client := NewClientWithOptions(ClientOptions{LookPathFn: func(file string) (string, error) { return "/usr/local/bin/xlsx-review", nil }})
