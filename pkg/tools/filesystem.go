...
var (
	...
	readFileBlockedExtensions = map[string]string{
		...
		".xlsx": "Use `xlsx_review_read`/`xlsx_review_diff`/`xlsx_review_apply` or `xlsx-review` instead of raw bytes.",
		".xls":  "Convert legacy `.xls` files to `.xlsx` or use spreadsheet tooling instead of raw bytes.",
		".pptx": "Use `pptx_review_read`/`pptx_review_diff`/`pptx_review_apply` or `pptx-review` instead of raw bytes.",
		".ppt":  "Convert legacy `.ppt` files to `.pptx` or use a presentation extraction workflow instead of raw bytes.",
