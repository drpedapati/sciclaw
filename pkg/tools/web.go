...
func (t *WebSearchTool) Description() string {
	return "Search the web for current information. Returns titles, URLs, and snippets from search results. Prefer `weather_forecast` for weather questions. Do not use this as the first step for PubMed citation verification when `pubmed_search` or `pubmed_fetch` can answer the query directly."
}
...
func (t *WebFetchTool) Description() string {
	return "Fetch a URL and extract readable content (HTML to text). Use this for general web pages like news and articles. Prefer `weather_forecast` for weather questions. Do not use this as the first step for PubMed citation verification or bibliographic lookup when `pubmed_search` or `pubmed_fetch` is available."
}
