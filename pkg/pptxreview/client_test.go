...
func TestClientResolveBinaryPathUsesSciClawTapInstallHint(t *testing.T) {
	client := NewClientWithOptions(ClientOptions{
		...
		BinaryCandidates: []string{"/missing/pptx-review"},
	})
	...
	if !strings.Contains(err.Error(), "brew tap drpedapati/tap && brew install sciclaw-pptx-review") {
		t.Fatalf("expected sciclaw tap install hint, got %v", err)
	}
	if strings.Contains(err.Error(), "brew tap drpedapati/tools && brew install pptx-review") {
