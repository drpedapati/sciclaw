...
func (e *CLIError) Error() string {
	if e == nil {
		return "pptx-review command failed"
	}
	...
	if source == "" {
		return fmt.Sprintf("pptx-review failed with exit code %d", e.ExitCode)
	}
	if len(source) > 400 {
		source = source[:400] + "... (truncated)"
	}
	return fmt.Sprintf("pptx-review failed with exit code %d: %s", e.ExitCode, source)
}

func NewClient() *Client { return NewClientWithOptions(ClientOptions{}) }
func NewClientWithOptions(opts ClientOptions) *Client {
	...
	return &Client{lookPathFn: lookPathFn, binaryCandidates: binaryCandidates, run: runFn, timeout: timeout}
}
...
func (c *Client) ResolveBinaryPath() (string, error) {
	if explicit := strings.TrimSpace(os.Getenv("PICOCLAW_PPTX_REVIEW_BINARY")); explicit != "" {
		if isExecutableBinary(explicit) {
			return explicit, nil
		}
	}
	if c.lookPathFn != nil {
		if binaryPath, err := c.lookPathFn("pptx-review"); err == nil && strings.TrimSpace(binaryPath) != "" {
...
func (c *Client) Apply(ctx context.Context, req ApplyRequest) (*ApplyResult, error) {
	...
	if req.OutputPath != "" && pathsReferToSameFile(req.InputPath, req.OutputPath) {
		return nil, fmt.Errorf("output path must differ from source presentation")
	}
	...
	result := &ApplyResult{ProcessingResult: parsed, Status: statusFromExitCode(res.ExitCode), ExitCode: res.ExitCode, DryRun: req.DryRun}
	if req.DryRun || req.OutputPath == "" {
		return result, nil
	}
	...
	if expectsOutputFile(result) {
		if statErr != nil {
			return nil, fmt.Errorf("pptx-review completed without producing output file %q: %w", req.OutputPath, statErr)
		}
		if beforeErr == nil && sameFileSnapshot(beforeInfo, afterInfo) {
			return nil, fmt.Errorf("pptx-review reported changes but did not update output file %q", req.OutputPath)
		}
...
func (c *Client) runJSON(ctx context.Context, args []string, dst interface{}, allowJSONOnExitOne bool) (RunResult, error) {
	...
	if runErr != nil {
		...
		return res, fmt.Errorf("pptx-review invocation failed: %w", runErr)
	}
	if res.ExitCode != 0 {
		if allowJSONOnExitOne && res.ExitCode == 1 {
			if jsonErr := json.Unmarshal([]byte(res.Stdout), dst); jsonErr == nil {
	...
	if err := json.Unmarshal([]byte(res.Stdout), dst); err != nil {
		return res, fmt.Errorf("pptx-review returned invalid JSON: %w", err)
	}
	return res, nil
}
...
func defaultBinaryCandidates() []string {
	...
	if runtime.GOOS == "windows" {
		binName = "pptx-review.exe"
	}
	candidates := make([]string, 0, 6)
	candidates = add(candidates, os.Getenv("PICOCLAW_PPTX_REVIEW_BINARY"))
	...
	case "windows":
		if local := strings.TrimSpace(os.Getenv("LOCALAPPDATA")); local != "" {
			candidates = add(candidates, filepath.Join(local, "Programs", "pptx-review", binName))
		}
		if pf := strings.TrimSpace(os.Getenv("ProgramFiles")); pf != "" {
			candidates = add(candidates, filepath.Join(pf, "pptx-review", binName))
		}
		if pfx86 := strings.TrimSpace(os.Getenv("ProgramFiles(x86)")); pfx86 != "" {
			candidates = add(candidates, filepath.Join(pfx86, "pptx-review", binName))
		}
	}
	return candidates
}
